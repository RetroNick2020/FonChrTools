...
{
  Windows FON Font Reader Library for Free Pascal - to be used with foninfo only at the moment
  Need to make draw functions virtual so they can be used in many environments. There is a USE_PTCGRAPH
  define but currently does not do anything. if you need FON support for ptcgraph please you use
  fonlibrary in one of my other repositories. 
...
type
  // Simple bitmap structure without Graphics unit dependency
  TPixelBuffer = record
    Width, Height: Integer;
    Data: PByte;    // Pointer to raw pixel data (BGRA format)
    RowStride: Integer; // Bytes per row (typically Width * 4)
  end;
  ...
  public
    ...
    // Drawing functions
    procedure DrawRasterCharInTextMode(CharIdx: Integer);
...
//    function GetGlyphBitmap(CharCode: Integer; var Buffer: TPixelBuffer): Boolean;
...
procedure TWinFont.DrawRasterCharInTextMode(CharIdx: Integer);
var
  Row, Col: Integer;
  ByteIdx, BitIdx: Integer;
  ByteVal: Byte;
...
(*
function TWinFont.GetGlyphBitmap(CharCode: Integer; var Buffer: TPixelBuffer): Boolean;
var
  Row, Col, Plane, ByteIdx, BitIdx: Integer;
  ByteVal: Byte;
  PixelPtr: PByte;
begin
  ...
  if (Buffer.Width <> FGlyphs[CharCode].Width) or
     (Buffer.Height <> FGlyphs[CharCode].Height) then
  begin
    Buffer.Width := FGlyphs[CharCode].Width;
    Buffer.Height := FGlyphs[CharCode].Height;
    Buffer.RowStride := Buffer.Width * 4; // 4 bytes per pixel (BGRA)
    // Caller must allocate Data buffer with: GetMem(Buffer.Data, Buffer.Height * Buffer.RowStride)
  end;

  // Clear to white (BGRA: 255,255,255,255)
  if Buffer.Data <> nil then
  begin
    FillByte(Buffer.Data^, Buffer.Height * Buffer.RowStride, $FF);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (ByteVal and (1 shl BitIdx)) <> 0 then
        begin
          // Set pixel to black (BGRA: 0,0,0,255)
          PixelPtr := Buffer.Data + (Row * Buffer.RowStride) + (Col * 4);
          PixelPtr^ := 0;       // Blue
          (PixelPtr + 1)^ := 0; // Green
          (PixelPtr + 2)^ := 0; // Red
